module IntOps = struct
  type 'a t = [ `IntAdd of 'a * 'a | `IntLit of int ] [@@deriving eq, show]
  type 'a intops = 'a t
  ...
  let rec map_only_intops f : 'a intops -> 'b intops =
    map_intops map_only_intops f

  let () =
    print_endline
    @@ show Format.pp_print_string
    @@ map_only_intops Int.to_string (`IntAdd (1, 2))
...
module BvOps = struct
  type 'a t = [ `BvAdd of 'a * 'a | `BvLit of int * int ] [@@deriving eq, show]
  type 'a bvops = 'a t
...
module AllOps = struct
  type 'a t = [ 'a IntOps.t | 'a BvOps.t ] [@@deriving eq, show]
  type 'a allops = 'a t
...
struct
  type t = Fix of t M.t [@@deriving eq, show]
